...
class QuestaoViewController: UIViewController {
    ...
    @IBAction func respostaBotaoPressionado(_ sender: UIButton) {
        ...
        if numeroQuestao < questoes.count - 1 {
            numeroQuestao += 1
            Timer.scheduledTimer(timeInterval: 0.4, target: self, selector: #selector(configurarQuestao), userInfo: nil, repeats: false)
        } else {
    ...
    @objc func configurarQuestao() {
        ...
        for botao in botoesRespostas {
            let tituloQuestao = questoes[numeroQuestao].respostas[botao.tag]
            botao.setTitle(tituloQuestao, for: .normal)
